...
├── ldap-groups.d/               # LDAP group-change hooks (*.hook files executed on mutations)
...
| `ldap.yml`           | Deploys OpenLDAP; creates `/symbios/ldap-groups.d/` for group-change hooks. |
...
- Creates `/symbios/ldap-groups.d/` — drop `*.hook` files here to react to
  LDAP group or membership changes. Hooks are executed by the shared dispatcher
  `f_ldap_groups_hooks` (in `scripts/symbios-lib.sh`) after every successful
  mutation in `symbios-ldap-groups.sh`. Each hook receives three arguments:
  `$1` = event (`group-created`, `group-deleted`, `member-added`,
  `member-removed`), `$2` = group name, `$3` = uid (empty for create/delete).
  Hook errors are logged but never abort the calling mutation. See
  `nextcloud-admin-sync.hook` for a real-world example.
...
## 13. Feature plugin system

Complex services can expose configurable features through the WebUI using the
feature plugin system. Features are defined in a `plugin.yml` manifest inside
the service directory (`services/<name>/plugin.yml`).

### How it works

1. **Manifest**: `plugin.yml` defines features with their parameters, groups,
   and mapping to Ansible playbooks. Each feature has:
   - `id`, `name`, `icon`, `description`, `group`
   - `target`: `vm` (SSH into target) or `host` (runs locally)
   - `playbook`: path to the Ansible playbook in `features/`
   - `params`: list of configurable parameters (type: text, password, select, multi-select, number)
   - `param_mapping`: maps WebUI parameter names to Ansible extra-vars

2. **State**: `features-state.yml` tracks each feature's status (draft/applied/error),
   enabled/disabled toggle, and saved parameter values.

3. **Detection**: Each parameter can have a `detect` script that discovers available
   options (e.g., available WiFi radios, network interfaces). The WebUI calls
   `/plugins/<service>/features/<feature>/detect/<param>/` to populate dropdowns.
...
```
services/<name>/
  plugin.yml              # Feature manifest
  features-state.yml      # Current state (auto-generated by WebUI)
...
1. Add the feature definition to `plugin.yml` (see existing entries for format)
2. Create the playbook in `features/`
3. Optionally create a detect script in `features/` for dynamic parameters
4. The feature appears automatically in the WebUI Features tab

### Currently implemented

- **openwrt-vm**: Network interfaces, WiFi AP, IoT WiFi, WiFi passthrough,
  WireGuard VPN, DNS over HTTPS, DynDNS, DHCP hosts, DNS records, SSH,
  Syslog, SMTP.
